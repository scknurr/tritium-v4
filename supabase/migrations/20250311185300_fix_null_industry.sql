/*
  # Fix null industry handling

  Fixes:
  - Handle null industry_id values properly
  - Improve type casting for industry and category IDs
*/
      ...
      FOR old_value, new_value IN
        SELECT 
          ...
          CASE
            WHEN key = 'industry_id' THEN 
              'industry-' || COALESCE(
                (SELECT name FROM industries WHERE id = NULLIF((old_record->key)::text, 'null')::int),
                'none'
              ) ||
              '-to-' || COALESCE(
                (SELECT name FROM industries WHERE id = NULLIF((new_record->key)::text, 'null')::int),
                'none'
              )
            WHEN key = 'category_id' THEN
              'category-' || COALESCE(
                (SELECT name FROM skill_categories WHERE id = NULLIF((old_record->key)::text, 'null')::int),
                'none'
              ) ||
              '-to-' || COALESCE(
                (SELECT name FROM skill_categories WHERE id = NULLIF((new_record->key)::text, 'null')::int),
                'none'
              )
            ELSE
              key || '-' || 
              COALESCE((old_record->>key)::text, 'none') || 
              '-to-' || 
              COALESCE((new_record->>key)::text, 'none')
          END as change_key
